...
class SparkNeuSQL:
    ...
    @classmethod
    def get_spark_conf(cls, spark_conf_loc=None):
        if spark_conf_loc is None:
            path = os.path.join("conf", "spark_settings.conf") 
            spark_conf = path_resolver.resolve(path)
            return spark_conf

        return spark_conf_loc
    ...
    def spark_query(cls,
                   ...
                   debug=False,
                   print_stderr=True,
                   spark_config_loc = None,
                   **kwargs):
        """Executes a query using Spark CLI, captures stdout into a pd.DataFrame
        ...
        **kwargs will be directly passed to pd.read_csv()
        ...
        spark_config_loc = cls.get_spark_conf()
        logging.info("spark_config_loc: "+ spark_config_loc)    
        ...
        if return_data:
            call = call + 'NeuSQL -S -engine=spark -e "{}" -sc {}'.format(query, spark_config_loc)
        else:
            if staging_dir is None:
                staging_dir = Config.staging_loc_path()
            if staging_dir is not None:
                staging_dir = os.path.join(staging_dir, 'spark_query')
                os.makedirs(staging_dir, exist_ok=True)
                fname = '{}/tmp_spark_query_{}_{}_{}.sql'.format(staging_dir, started_on, current_process().ident, current_thread().ident)

            else:
                fname = 'tmp_spark_query_{}_{}.sql'.format(current_process().ident, current_thread().ident)
            logging.debug("Spark query staging loc:"+fname)

            with io.open(fname, 'w', encoding='utf-8') as f:
                f.write(query)
            call = call + 'NeuSQL -v -engine=spark -f {} -sc {}'.format(fname, spark_config_loc)
...
class TestSuite(unittest.TestCase):
    ...
    def test_get_dbs_df(self):
        data = SparkNeuSQL.hive_query(query='SHOW DATABASES;')
    ...
    def test_merge(self):
        ...
        data = SparkNeuSQL.hive_query(query='SELECT t1.id, t1.name, t2.age FROM hq_test.table1 t1 JOIN hq_test.table2 t2 ON t1.id=t2.id;')
